import type { AttendanceSource, AttendanceStatus } from "../../generated/prisma/client";
import { TeacherObservation } from "./dto";
...
export interface AttendanceCrossCheckRepository {
  withTransaction<T>(callback: (repo: AttendanceCrossCheckRepository) => Promise<T>): Promise<T>;
...
export class AttendanceCrossCheckService {
  ...
  resolveStatus(input: {
    ...
    if (input.teacherObservation === TeacherObservation.PRESENT) {
      return {
        status: "HADIR",
        source: "CROSS_CHECK"
      };
